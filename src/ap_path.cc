...
namespace
{
    ...
    void SortLinksbyMark(const double *min_delay_to_dst, const Flow &flow, Graph *graph)
    {
        for(Link &link: graph->GetMutableLinks())
        {
            link.weight = link.delay + min_delay_to_dst[link.ter_id];
            ...
            for(int i = 0; i < flow.SF_number; i++)
            {
                for(std::unordered_set<int>::const_iterator it = flow.Subset_sequence[i].begin();it!=flow.Subset_sequence[i].end();it++)
                {
                    if(link.ter_id == *it)
                    {
                        link.weight_mask = flow.SF_number-i;
                    }
                }
                
            }
        }
        graph->SortLinksMaskDelay();
    }
    ...
    while (!ap_stack.empty())
    {
        ...
        if (ap_snap.stage == 1)
        {
            ...
            if(ap_snap.flag == true){
                mask -= flow.node_to_mask.find(u)->second;
                step -= 1;
            }
            ap_visited[u] = false;
        }
        ...
        {
            int new_mask = mask;
            if(flow.node_to_mask.find(u)!=flow.node_to_mask.end())
            {
                if(mask+1 == flow.node_to_mask.find(u)->second)
                {
                    new_mask = mask + flow.node_to_mask.find(u)->second;
                }
            }
            double new_delay = ap_path_delay + ap_link->delay;
            double new_cost = ap_path_cost + ap_link->cost;
            ...
            else
            {
                ...
                // std::cout<<"now step: "<<step<<" "<<"now visit: "<<u<<" "<<"from "<<flow.node_mask_order[step]<<" to u: "<<step_delay;
                /*for(int i = step; i < flow.node_mask_order.size(); i++)
                {
                    step_delay += min_delay_to_dsts_[i+1][flow.node_mask_order[i]];
                    step_cost += min_cost_to_dsts_[i+1][flow.node_mask_order[i]];
                }*/
                for(std::unordered_map<NodeId,const double*>::const_iterator it= node_to_min_cost_to_dst_[step].begin();it!=node_to_min_cost_to_dst_[step].end();it++)
                {
                    step_delay = min(step_delay,DelayArray[it->first]+node_to_min_delay_to_dst_[step][it->first][u]);
                    step_cost = min(step_cost,CostArray[it->first] + node_to_min_cost_to_dst_[step][it->first][u]);
                }
                //std::cout<<step_delay<<" "<<step_cost<<std::endl;
                    ...
                    step_cost + new_cost < best_cost_so_far)
                {
                    ...
                    if (!egress_links.empty())
                    {
                        ...
                        if((flow.node_to_mask.find(u) != flow.node_to_mask.end()) && (mask + 1 == flow.node_to_mask.find(u)->second)){
                            mask = mask + flow.node_to_mask.find(u)->second;
                            step += 1;
                            ap_snap.flag = true;
                        }
                        else{
...
double MulGraphAp::FindOptPath(const Flow &flow, double cost_ub, LogInfo &ap_info, LogInfo &bp_info)
{
    ...
    double ap_path_cost = 0;
    ap_path.reserve(100);
    std::vector<bool> ap_visited((graph_->GetMaxNodeId() + 1)*(flow.SF_number+1), false);
    //std::vector<bool> hash_ap_visited(graph_->GetMaxNodeId() + 1, false);
    visited_.clear();
    visited_.reserve(graph_->GetMaxNodeId() + 1);
    for(int i = 0; i < graph_->GetMaxNodeId() + 1; i++)
    {
    ...
    while (!ap_stack.empty())
    {
        ...
        if (ap_snap.stage == 1)
        {
            ...
            if(ap_snap.flag == true)
            {
                visited_[u%(graph_->GetMaxNodeId() + 1)] = 0;
            }
        }
        else
        {
            ...
            else
            {
                    ...
                    min_cost_to_dst_[u] + new_cost < best_cost_so_far)
                {
                    ...
                    if (!egress_links.empty())
                    {
                        ...
                        ap_path_cost += ap_link->cost;
                        ap_visited[u] = true;
                        if(graph_->CheckNodeConnect(u))
                        {
                            visited_[u % (graph_->GetMaxNodeId() + 1)] = 1;
                        }else{
                            visited_[u % (graph_->GetMaxNodeId() + 1)] = 2;
                        }
                        ap_snap.stage = 1;
                        ap_stack.push_back(ap_snap);
                        for (Link *link : egress_links)
                        {
                            ...
                            snap.link = link;
                            snap.flag = true;
                            if(visited_[node_id % (graph_->GetMaxNodeId() + 1)] == 2){
                                continue;
                            }else if(visited_[node_id % (graph_->GetMaxNodeId() + 1)] == 1){
                                if(!graph_->CheckNodeConnect(node_id)){
                                    continue;
                                }else{
                                    snap.flag = false;
                                }
