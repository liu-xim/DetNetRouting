...
Path MaskPulse::FindPath(const Flow& flow)
{
    clock_t start_time;
    clock_t end_time;
    start_time = clock();
    MaskAp ap;
    std::vector<std::unordered_map<int,const double*>> node_min_costs;
    std::vector<std::unordered_map<int,const double*>> node_min_delay;
    for(int i = 0; i < flow.SF_number; i++)
    {
        node_min_costs.push_back(std::unordered_map<int,const double*>());
        node_min_delay.push_back(std::unordered_map<int,const double*>());
        for(std::unordered_set<int>::const_iterator it = flow.Subset_sequence[i].begin();it!=flow.Subset_sequence[i].end();it++)
        {
            ...
            AStar a_star_delay(graph_, LinkDelay);
            AStar a_star_cost(graph_, LinkCost);
            a_star_delay.InitWithDst(node,nullptr);
            a_star_cost.InitWithDst(node,nullptr);
            double* temp_delay = new double[graph_->GetMaxNodeId()+1];
            double* temp_cost = new double[graph_->GetMaxNodeId()+1];
            for(int i = 0; i < graph_->GetMaxNodeId()+1; i++)
            {
                temp_delay[i] = a_star_delay.GetCostVector()[i];
                temp_cost[i] = a_star_cost.GetCostVector()[i];
            }
            node_min_costs[i].insert(std::pair<int,const double*> {node,temp_cost});
            node_min_delay[i].insert(std::pair<int,const double*> {node,temp_delay});
        }
    }
    node_min_costs.push_back(std::unordered_map<int,const double*>());
    node_min_delay.push_back(std::unordered_map<int,const double*>());
    AStar a_star_delay(graph_, LinkDelay);
    AStar a_star_cost(graph_, LinkCost);
    a_star_delay.InitWithDst(flow.to,nullptr);
    a_star_cost.InitWithDst(flow.to,nullptr);
    node_min_costs.back().insert(std::pair<int,const double*> {flow.to,a_star_cost.GetCostVector()});
    node_min_delay.back().insert(std::pair<int,const double*> {flow.to,a_star_delay.GetCostVector()});
    ap.Init(graph_, nullptr, flow,node_min_costs,
            node_min_delay);
    double min_cost = ap.FindOptPath(flow, kMaxValue, ap_info_, bp_info_);
    Path result;
    if (min_cost < kMaxValue) {
...
Path MulGraphPulse::FindPath(const Flow& flow)
{
    ...
    Flow flow_ = flow;
    int original_size = (graph_->GetMaxNodeId()+1);
    flow_.to = flow.to + flow.SF_number*original_size;
    MulAStar a_star_delay(graph_, LinkDelay);
    a_star_delay.InitWithDst(flow_.to);
    MulAStar a_star_cost(graph_, LinkCost);
    a_star_cost.InitWithDst(flow_.to);
    MulGraphAp ap;
    ap.Init(graph_, nullptr, a_star_cost.GetCostVector(),
            a_star_delay.GetCostVector());
    double min_cost = ap.FindOptPath(flow_, kMaxValue, ap_info_, bp_info_);
    Path result;
    if (min_cost < kMaxValue)
    {
        std::vector<Link*> mullink = ap.GetApPath();
        std::vector<Link*> simlink;
        simlink.reserve(mullink.size());
        for(int i = 0; i < mullink.size();i++)
        {
            if(mullink[i]->link_id >= (graph_->GetMaxLinkId()+1) * (flow.SF_number+1)){
                continue;
            }else{
                simlink.push_back(new Link((mullink[i]->link_id)%(graph_->GetMaxLinkId()+1),
                        (mullink[i]->source_id)%(graph_->GetMaxNodeId()+1),
                        (mullink[i]->ter_id)%(graph_->GetMaxNodeId()+1),
                        mullink[i]->cost,
                        mullink[i]->bandwidth,
                        mullink[i]->delay,
                        mullink[i]->srlg_num));
