...
namespace
{
    ...
    bool CompareMarkDelay(Link *a, Link *b)
    {
...
void Graph::SortLinksMaskDelay()
{
    for(NodeId i = 0; i < size_; ++i)
    {
        std::sort(node_to_egress_links_[i].begin(),
                  node_to_egress_links_[i].end(), CompareMarkDelay);
        std::sort(node_to_ingress_links_[i].begin(),
                  node_to_ingress_links_[i].end(), CompareMarkDelay);
    }
}
