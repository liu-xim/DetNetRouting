...
//     return result;
// }
Path SfcKsp::FindPath(const Flow& flow)
{
    clock_t start_time;
    clock_t end_time;
    start_time = clock();
    ...
    while (cost < kMaxValue)
    {
        ...
        const std::vector<Link *> &cur_path = ksp.GetPath();
        double delay = ComputeDelay(cur_path);
        if (flow.CheckDelayUb(delay) && flow.CheckDelayLb(delay) && flow.CheckSfc(cur_path))
        {
            result.path_link = cur_path;
            result.CompletePath();
            break;
...
Path SfcLagrangianKsp::FindPath(const Flow &flow)
{
    clock_t start_time;
    clock_t end_time;
    start_time = clock();
